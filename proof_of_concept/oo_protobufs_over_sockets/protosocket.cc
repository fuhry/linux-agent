...
bool ProtoSocket::OpenTCPListener(int port)  {
  ...
  bool ProtoSocket::OpenUNIXConnection(string &unix_path) {
    ...
    addr.sun_family = AF_UNIX;
    strcpy(addr.sun_path, unix_path_.c_str());
  ...
  bool ProtoSocket::OpenTCPConnection(string &remote_server, uint16_t remote_port) {
    ...
    if ( getaddrinfo(remote_server.c_str(), strport, &hints, &ai_res) < 0 ) {
  ...
  unique_ptr<ProtoSocket> ProtoSocket::NewConnection() {
    ...
    ProtoSocket * new_ps = new ProtoSocket(newfd);
    unique_ptr<ProtoSocket> tps;
    tps.reset(new_ps);
  ...
  bool ProtoSocket::SendMessage(ProtoSocketMessage &psm) {
    ...
    return SendMessage(psm.size, std::move(psm.buffer));
  ...
  bool ProtoSocket::SendMessage(uint32_t size, unique_ptr<unsigned char> buffer) {

    int msg_size = size;
    unsigned char *t = buffer.get();;
    ...
    bytes_sent = send(fd_, t, msg_size, MSG_NOSIGNAL);
  ...
  unique_ptr<ProtoSocketMessage> ProtoSocket::GetMessage() {

    unique_ptr<ProtoSocketMessage> psm;
    ProtoSocketMessage *tpsm;
    uint32_t msg_len_net, msg_len_int;
    ssize_t  bytes_read;
    unique_ptr<unsigned char> buffer;

    tpsm = new ProtoSocketMessage;
    tpsm->size = 0;
    psm.reset(tpsm);
    ...
    //DEBUG
    cerr << "when trying to read 4 bytes for length, read " << bytes_read << endl;
