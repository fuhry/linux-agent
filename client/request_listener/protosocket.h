...
class ProtoSocket {
  ...
  public:
    ...
    shared_ptr<ProtoSocketMessage> GetMessage();   //  Read a message.  CAN BLOCK!

    bool SendMessage(ProtoSocketMessage &psm);
    bool SendMessage(ProtoSocketMessage *psm);
    bool SendMessage(uint32_t size, shared_ptr<unsigned char>  buffer);
    bool SendMessage(uint32_t size, unsigned char *  buffer);

    shared_ptr<ProtoSocket> NewConnection();   //  accept(), etc.  CAN BLOCK!
